...
/**
 * <p>
 * This class allows us to approximate the trail of fish particles made by a fish converging to the player's
 * bobber. To do this, we use a variant on the <strong>Bellman Ford</strong> algorithm, as well as the
 * <strong>exponentiation-by-squaring</strong> algorithm.
 * </p>
 *
 * <p>
 * In general, we wish to identify those splash particles that belong to a fish converging on the player's
 * bobber. Notice several identifying features of these converging particles:
 * </p>
 *
 * <ol>
 *   <li>Particles spawn at a radial distance from the cast hook.
 *     <ul>
 *       <li>That distance decreases by 0.1 blocks for sequential particles (with a small amount of variation).</li>
 *       <li>During the rain, or in the case of dropped packets, the distance can be 0.2 blocks.</li>
 *     </ul>
 *   </li>
 *   <li>Particles spawn at an angle (think polar coordinates) to the cast hook.
 *     <ul>
 *       <li>That angle differs based on a gaussian distribution for sequential particles.</li>
 *       <li>The gaussian distribution has mean 0 and standard deviation 4 degrees.</li>
 *       <li>Based on the distribution, very few sequential particles (&lt;0.27%) will differ in angle by more
 *           than 3 standard distributions (12 degrees).</li>
 *     </ul>
 *   </li>
 *   <li>Sequential particles approaching the hook generally come in every tick, but we can allow for 2 or 3
 *       ticks.</li>
 *   <li>Particles will not spawn farther than 8 blocks away from the cast hook.</li>
 * </ol>
 *
 * <p>
 * (These are mostly discoverable in the <code>EntityFishHook.java</code> file. Hopefully, my making this algorithm
 * won't cause servers to go ballistic...)
 * </p>
 *
 * <p>
 * Consideration of time complexity is a huge part of the algorithm. It is an <code>O(n^2)</code> algorithm, which
 * for a large number of particles is unsustainable. Each tick only gives us 50 milliseconds to compute stuff, so
 * we're using bitwise operations. Unfortunately, the longest primitive is 64 bits, so we're limited to tracking
 * 64 particles at the moment. This means when 10+ fishers are in the same spot, we may not be able to link the
 * particles before we overwrite with new ones.
 * </p>
 *
 * <p>
 * Processing the per-particle step takes &lt;0.025 milliseconds and the per-tick step generally takes
 * &lt;0.050 milliseconds. So we are well below any critical thresholds for computation time.
 * </p>
 *
 * @author Phoube
 */
public class FishParticleManager {
     ...
     */
    private static final double DIST_VARIATION = .02;
    /**
    ...
    private static final ArrayList<Particle>[] particleList = new ArrayList[64];
    /**
     * A set of the positions (designed to filter out the double particles that spawn at the same position)
     */
    private static final Double2ObjectLinkedOpenHashMap<List<Particle>> particleHash = new Double2ObjectLinkedOpenHashMap<>(64);
    /**
     ...
     */
    public static void onFishWakeSpawn(WakeParticle fishWakeParticle) {
        ...
        if (hook != null && !particleHash.containsKey(hash)) {
            double distToHook = Math.sqrt((xCoord - hook.getX()) * (xCoord - hook.getX()) + (zCoord - hook.getZ()) * (zCoord - hook.getZ()));
            ...
            particleAngl[idx] = Mth.atan2(xCoord - hook.getX(), zCoord - hook.getZ()) * 180 / Math.PI;
            particleTime[idx] = level.getGameTime();
            // We want O(1) index lookup and position-hash lookup. Use hashmap and array
            ArrayList<Particle> tmp = new ArrayList<>(Collections.singletonList(fishWakeParticle));
            ...
            if (particleHash.size() > 64) {
                ObjectBidirectionalIterator<Double2ObjectMap.Entry<List<Particle>>> itr = particleHash.double2ObjectEntrySet().iterator();
    ...
    private static void calculateTrails() {
        ...
        boolean first = true;
        long currTick = MC.level.getGameTime();
